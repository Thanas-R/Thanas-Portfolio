...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center px-6">
      <div className="max-w-5xl w-full mx-auto grid md:grid-cols-2 gap-6 items-center">
        <motion.div
          initial={{ x: -60, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.8, ease: [0.25, 0.1, 0, 1] }}
        >
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-bold text-foreground leading-[0.85] tracking-tight font-['Space_Grotesk']">
            thanas{' '}
            <span className="relative inline-flex items-baseline">
              r
              <span
                className={`absolute bottom-[-3px] right-[-10px] w-3.5 h-3.5 md:w-4 md:h-4 rounded-full ring-[5px] ring-background ${statusColor} cursor-pointer transition-colors duration-500`}
                title={discordStatus === 'offline' ? 'Offline' : `${discordStatus} on Discord`}
              />
            </span>
          </h1>
          ...
          <div className="[perspective:1000px]">
            <div
              onMouseLeave={() => {
                ...
                const el = document.getElementById('hero-pfp');
                if (el) {
              ...
              onMouseMove={(e) => {
                ...
                const el = document.getElementById('hero-pfp');
                if (el) {
                  el.style.transform = `rotateX(${xRot}deg) rotateY(${yRot}deg) scale(1.05)`;
                }
              }}
              id="hero-pfp"
              className="relative w-52 h-52 md:w-64 md:h-64 glow-card rounded-2xl transition-transform duration-200 ease-out"
            >
              <img
                src={avatar}
                alt="thanas r"
