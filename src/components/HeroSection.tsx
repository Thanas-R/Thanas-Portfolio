              ...
              <TooltipProvider delayDuration={150} skipDelayDuration={200}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    {/* status-dot: uses em units so it scales with the font-size of the 'R' */}
                    <span
                      aria-label={statusLabel}
                      className={`status-dot ${statusColor} cursor-pointer`}
                      style={
                        {
                          // dot size relative to font-size of the 'R'
                          ['--dot-size' as any]: '0.65em',
                          // ring thickness factor (multiply by dot-size)
                          ['--ring-factor' as any]: '0.28',
                          // ring color — keep as background variable (adjust in CSS if you use a different background token)
                          ['--ring-color' as any]: 'var(--background, #fff)'
